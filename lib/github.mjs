...
export async function loadRepositoryIssue(input, {fetchImpl = fetch} = {}) {
  ...
  async function read(path) {
    ...
    try {return await readJson(response, {maxBytes: 8 * 1024 * 1024, signal});}
    catch {throw new Error('Could not read the GitHub response within the time or 8 MiB size limit.');}
  }
  ...
  for (let page = 1; page <= LIMITS.poolPages; page++) {
    ...
    if (!Array.isArray(data) || data.length > 100) throw new Error('GitHub returned an invalid issue list.');
