...
export function normalizeIssue(raw) {
  ...
  return {
    ...
    labels: (raw.labels ?? []).map(x => typeof x === 'string' ? x : x?.name).filter(x => typeof x === 'string').slice(0, 10),
...
export function rankCandidates(source, issues, limit = LIMITS.candidates) {
  ...
  const termSets = docs.map(x => new Set(tokens(`${x.title} ${x.body}`)));
  const df = new Map();
  for (const set of termSets) for (const term of set) df.set(term, (df.get(term) ?? 0) + 1);
  return docs.map((issue, index) => {
    const titleTerms = new Set(tokens(issue.title));
    let score = 0;
    for (const term of query) if (termSets[index].has(term)) {
      score += Math.log(1 + docs.length / (1 + df.get(term))) * (titleTerms.has(term) ? 2 : 1);
    }
    score /= Math.sqrt(1 + termSets[index].size / 100);
    const overlap = [...query].filter(t => termSets[index].has(t)).slice(0, 8);
